  ...
  var ProductVariants = function () {
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _onVariantChanged(previousVariant, newVariant) {
        ...
        // 5th: Update product gallery
        this._updateProductGallery(newVariant);
      ...
      key: '_updateProductGallery',
      value: function _updateProductGallery(newVariant) {
        if (!newVariant) return
        const variantUrl = window.location.pathname + `?variant=${newVariant.id}&section_id=product-template`
        fetch(variantUrl)
          .then(response => response.text())
          .then(text => {
            const html = new DOMParser().parseFromString(text, 'text/html');
            const productGallerySource = this.element.querySelector('.Product__Gallery');
            ...
            if (productGallerySource && productGalleryNew) {
              ...
              const galleryElem = document.querySelector('.ProductMain .Product__Slideshow');
              const thumbnailsElem = document.querySelector('.ProductMain .Product__SlideshowNavScroller');
              if (galleryElem) {
              ...
              if (thumbnailsElem) {
                ...
                if (thumbnailsConfig) {
                  const galleryWrapper =  document.querySelector('.Product__Gallery');
      ...
      value: function _updateAddToCartButton(newVariant) {
        var addToCartButton = this.element.querySelector('.ProductForm__AddToCart'),
            ...
            messageAvailable = document.querySelector('.message-delivery'),
            notifyVariantNotifyForm = document.querySelector('[data-notify-variant-detail]'),
            notifyMeForm = document.querySelector('[data-notify-product]');


        newButton.setAttribute('type', 'submit');
        newButton.className = 'ProductForm__AddToCart Button Button--full';

        notifyVariantNotifyForm.value = newVariant['id'];
        ...
        if (!newVariant) {
          ...
          messageAvailable.style.display = 'none';
          notifyMeForm.classList.remove('hide');
          ...
          if (newVariant['available']) {
            ...
            messageAvailable.style.display = 'block';
            notifyMeForm.classList.add('hide');

            if (undefined === this.options['showPriceInButton'] || this.options['showPriceInButton']) {
              newButton.innerHTML = '\n            <span>' + window.languages.productFormAddToCart + '</span>\n            <span class="Button__SeparatorDot"></span>\n            <span data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(newVariant['price'], window.theme.moneyFormat) + '</span>\n          ';
            } else {
              newButton.innerHTML = '<span>' + window.languages.productFormAddToCart + '</span> &nbsp; <span> - </span> &nbsp; <span data-money-convertible>' + __WEBPACK_IMPORTED_MODULE_4__helper_Currency__["default"].formatMoney(newVariant['price'], window.theme.moneyFormat) + '</span>';
            }
          } else {
            newButton.setAttribute('disabled', 'disabled');
            newButton.classList.add('Button--primary');
      ...
      value: function _addToCart(event) {
        ...
        var variantId = __WEBPACK_IMPORTED_MODULE_2__helper_Form__["default"].serialize(formElement).id;
        var variantInventoryQuantity = parseInt($('.variant-inventory-quantity').find('#variant-' + variantId).text());
        var quantityAdd = parseInt(formElement.querySelector('[name="quantity"]').value);
        ...
        fetch(window.routes.cartAddUrl + '.js', {
          ...
          if (response.ok) {
            ...
            response.json().then(function (content) {
              var lineItemQuantity = $('.CartItem__QuantitySelector').find('.CartItem__MaxQuantity[data-line-item-id="' + variantId + '"]').attr('data-line-item-quantity');
              var contentDescription = content['description'];
              console.log(lineItemQuantity);
              if (contentDescription.includes('You can\'t add more') || contentDescription.includes('are in your cart.')) {
                if (variantInventoryQuantity == 1) {
                  contentDescription = "Looks like we only have 1 unit left and it's already in your cart!";
                } else if (variantInventoryQuantity < quantityAdd) {
                  contentDescription = "Looks like we only have " + variantInventoryQuantity + " units left!";
                } else if (variantInventoryQuantity > 1) {
                  contentDescription = "Looks like we only have " + variantInventoryQuantity + " and there are " + lineItemQuantity + " in your cart!";
                } else {
                  contentDescription = "Sorry, looks like we don't have enough of this product. Please try adding fewer items to your cart.";
                }
                // contentDescription = "Sorry, looks like we don't have enough of this product. Please try adding fewer items to your cart.";
              }
              var errorMessageElement = document.createElement('span');
              errorMessageElement.className = 'ProductForm__Error Alert Alert--error';
              errorMessageElement.innerHTML = contentDescription;
              addToCartButton.removeAttribute('disabled');
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var SearchBar = function () {
    function SearchBar() {
      ...
      this.searchElementDesk = document.getElementById('SearchDesk');
      this.searchInputElementDesk = this.searchElementDesk.querySelector('[name="qq"]');
      this.searchCloseElementDesk = this.searchElementDesk.querySelector('.Search__Close');
      this.searchOpenElementDesk = this.searchElementDesk.querySelector('.Search__InputIconWrapper');
    ...
    _createClass(SearchBar, [{
      ...
      value: function destroy() {
        ...
        this.searchInputElementDesk.removeEventListener('keydown', this._preventSubmissionListener);
        this.searchInputElementDesk.removeEventListener('input', this._onInputListener);
      ...
      value: function _attachListeners() {
        ...
        this.searchInputElementDesk.addEventListener('keydown', this._preventSubmissionListener);
        this.searchInputElementDesk.addEventListener('input', this._onInputListener);

        this.searchInputElementDesk.addEventListener('click', this._openSearchDesk.bind(this));

        this.documentDelegate.on('click', '[data-action="toggle-search"]', this._toggleSearch.bind(this));
        this.documentDelegate.on('click', '[data-action="open-search"]', this._openSearch.bind(this));
        this.documentDelegate.on('click', '[data-action="close-search"]', this._closeSearch.bind(this));
        this.documentDelegate.on('click', '[data-action="close-search-desk"]', this._closeSearchDesk.bind(this));

        this.documentDelegate.on('search:close', this._closeSearch.bind(this)); // Allow for third-party elements to close the bar
        this.documentDelegate.on('search:close', this._closeSearchDesk.bind(this)); // Allow for third-party elements to close the bar
      }
      ...
      key: '_searchDeskFocus',
      value: function _searchDeskFocus(event) {

        if (this.isOpen) {
          this._closeSearchDesk(event);
        } else {
          this._openSearchDesk(event);
        }
      ...
      key: '_openSearchDesk',
      value: function _openSearchDesk() {
        var _this20 = this;

        this.searchElement.setAttribute('aria-hidden', 'false');

        document.documentElement.classList.add('no-scroll');
        __WEBPACK_IMPORTED_MODULE_0__helper_Accessibility__["default"].trapFocus(this.searchElementDesk, 'search', this.searchElementDesk.querySelector('[name="qq"]'));

        var onFocusListener = function onFocusListener() {
          _this20.searchInputElementDesk.focus();
          _this20.searchElementDesk.removeEventListener('transitionend', onFocusListener);
        };

        this.searchOpenElementDesk.classList.add('hide-icon');
        this.searchCloseElementDesk.classList.remove('hide-icon');

        this.searchInputElementDesk.focus();
        this.searchElementDesk.addEventListener('transitionend', onFocusListener);
        this.isOpen = true;
      ...
      key: '_closeSearchDesk',
      value: function _closeSearchDesk() {
        var _this21 = this;

        this.searchElement.setAttribute('aria-hidden', 'true');

        document.documentElement.classList.remove('no-scroll');
        __WEBPACK_IMPORTED_MODULE_0__helper_Accessibility__["default"].removeTrapFocus(this.searchElementDesk, 'search');
        ...
        this.searchOpenElementDesk.classList.remove('hide-icon');
        this.searchCloseElementDesk.classList.add('hide-icon');

        this.searchInputElementDesk.addEventListener('focusout', onTransitionEnd);
        this.pageOverlayElement.addEventListener('transitionend', onTransitionEnd);
        this.pageOverlayElement.classList.remove('is-visible');
      }
  ...
  var ArticleSection = function () {
    ...
    _createClass(ArticleSection, [{
      ...
      value: function _checkToolbarVisibility() {
        ...
        fastdom.mutate(function () {
          if (_this28.toolbarElement != null) {
          ...
          var playerLoadingInterval = setInterval(function () {
            if (_this31.options['videoType'] === 'youtube') {
              ...
              if (window.Vimeo) {
                ...
                setTimeout(function (){_this31.player.pause();}, 1000);

                if (_this31.isInViewport(_this31.element)) {
                  _this31.player.play();
                }

                window.addEventListener('scroll', function () {
                  if (_this31.isInViewport(_this31.element)){_this31.player.play();}
                });
              }
            }
          }, 50);
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var CartSection = function () {
    function CartSection(container) {
      ...
      const openDrawer = window.location.toString().includes("openDrawer");
    ...
    _createClass(CartSection, [{
      ...
      value: function _attachListeners() {
        ...
        this.delegateElement.on('click', '.CartItem__Actions a', this._disableCheckout.bind(this));
      ...
      value: function _rerenderCart(elementToAnimate) {
        ...
        return fetch(window.theme.localeRootUrl + `?sections=cart-drawer` + '&timestamp=' + Date.now(), {
          credentials: 'same-origin',
          method: 'GET'
        }).then((response) => {
          return response.json();
        }).then((response) => {
          return new DOMParser().parseFromString(response['cart-drawer'], 'text/html')
        }).then(function (content) {
          const html = content.querySelector('#sidebar-cart').parentNode;
      ...
      value: function _replaceContent(html) {
        ...
        _this36._checkGWP(html);

        var tempElement = document.createElement('div');
        tempElement.innerHTML = html.innerHTML;
        ...
        cartDot.forEach(function (item) {
          if (_this36.itemCount === 0) {
            ...
            const GWPNotification = html.querySelector('.Drawer__GiftWithProductMessage');
            if (GWPNotification) {
              document.querySelector('.Drawer__GiftWithProductMessage').innerHTML = GWPNotification.innerHTML;
            }
          } else {
      ...
      {
        key: '_updateGwp',
        value: function _updateGwp(data) {
          var  _this34 = this;
          ...
          fetch(window.theme.localeRootUrl + '/cart/update.js', {
            ...
            _this34._rerenderCart();
          });
        }
      },
      {
        key: '_addGwp',
        value: function _addGwp(data) {
          var  _this34 = this;
      ...
      {
        key: '_checkGWP',
        value: function _checkGWP(html) {
          var _this35 = this;

          const giftProductData = JSON.parse((html.querySelector('[data-gift-product]')).textContent);
          
          const {
            gwp_enabled: gwpEnabled,
            gwp_items_for_add: gwpItemsForAdd,
            gwp_items_for_remove: gwpItemsForRemove,
            gwp_items_in_cart_keys: gwpItemsInCartKeys,
            gwp_items_in_cart_ids: gwpItemsInCartIds,
            gwp_items_in_cart_for_update_qty_keys: gwpItemsInCartForUpdateQtyKeys,
            gwp_potential_variants_ids_in_cart: gwpPotentialVariantsIdsInCart
          } = giftProductData;

          if (gwpItemsInCartForUpdateQtyKeys.length) {
            const data = {};
            gwpItemsInCartForUpdateQtyKeys.forEach(key => data[`${key}`] = 0);
            _this35._updateGwp(data);
            console.log('update');
            return false;
          }

          const gwpPotentialVariantsPresent = gwpItemsInCartIds.every(elem => gwpPotentialVariantsIdsInCart.includes(elem));

          if (gwpItemsInCartKeys.length
            && (!window.sessionStorage.gwpClear
              || !gwpEnabled
              || (!gwpPotentialVariantsPresent && !gwpItemsForRemove.length)
            )) {
            const data = {};
            gwpItemsInCartKeys.forEach(key => data[`${key}`] = 0);
            _this35._updateGwp(data);
            window.sessionStorage.gwpClear = true;
            console.log('clear');
            return false;
          }

          if (gwpItemsForAdd.length) {
            console.log('add')
            const data = gwpItemsForAdd.map(el => {
              return {
                id: el,
                quantity: 1,
                properties: {"_GWP product": "true"}
              }
            })
            _this35._addGwp(data);

            return false;
          } else if (gwpItemsForRemove.length) {
            console.log('remove')
            const data = {};
            gwpItemsForRemove.forEach(key => data[`${key}`] = 0);
            _this35._updateGwp(data);
      ...
      {
        key: '_disableCheckout',
        value: function _disableCheckout() {
          document.querySelector('.Cart__Checkout').classList.add('Cart__Checkout--disabled');

          console.log('Cart__Checkout--disabled');
        }
      }
    ]);
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var CollectionSection = function () {
    function CollectionSection(element) {
      ...
      var paginatePopoverElement = document.getElementById('dynamic-pagination');
      if (paginatePopoverElement) {
        this.sortPopover = new __WEBPACK_IMPORTED_MODULE_1__components_Popover__["default"](paginatePopoverElement, { onValueChanged: this._paginateByChanged.bind(this) });
      }
    ...
    _createClass(CollectionSection, [{
      ...
      value: function _sortByChanged(sortBy) {
        ...
        document.querySelector('.CollectionToolbar__Item--sort span').textContent = document.querySelector('[data-value=' + sortBy + ']').textContent;

        this.currentSortBy = sortBy;
        this._reloadProducts();
      }
    }, {
      key: '_paginateByChanged',
      value: function _paginateByChanged(val) {
      ...
      value: function _reloadProducts() {
        ...
        var collectionQueryAjaxTemplate;
        var collectionTemplateNoFilterTrue = document.querySelector('[data-section-type="collection"]').classList.contains('no-filter');

        if (!collectionTemplateNoFilterTrue) {
          collectionQueryAjaxTemplate = 'ajax';
        } else {
          collectionQueryAjaxTemplate = 'ajax-2';
        }

        var formData = new FormData();
        formData.append('view', collectionQueryAjaxTemplate);
        formData.append('sort_by', this.currentSortBy);

        fetch(location.pathname + '?view=' + collectionQueryAjaxTemplate + '&sort_by=' + this.currentSortBy, {
          credentials: 'same-origin',
          method: 'GET'
        }).then(function (response) {
          response.text().then(function (content) {
            ...
            if (!collectionTemplateNoFilterTrue) {
              ...
              _this42.collectionInnerElement.innerHTML = tempElement.querySelector('.shopify-section .CollectionInner__Products').innerHTML;
            }
            ...
            var divForPagination = document.querySelector('#for_pagination');
            divForPagination.insertBefore(pagination, divForPagination.firstChild);
            document.querySelector('.CollectionInner__Products .Pagination').remove();
          });
        });
      }
    }, {
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var HeaderSection = function () {
    function HeaderSection(container) {
      ...
      if (this.options['isSticky'] && $('#section-announcement').data('sticky')) {
        $('#shopify-section-header').addClass('sticky-with-announcement')
      }
    ...
    _createClass(HeaderSection, [{
      ...
      value: function _attachListeners() {
        ...
        document.querySelector(".Header__FlexItem--bottom").addEventListener('mouseleave', this._closeNavigationListener);

        this.delegateElement.on('mouseenter', '.Header__MainNav .HorizontalList__Item, [aria-haspopup]', this._openMenu.bind(this), true);
        this.delegateElement.on('mouseenter', '.Header__MainNav .HorizontalList__Item, [aria-haspopup]', this._firstActiveMegaMenuItem.bind(this), true);
      ...
      key: '_firstActiveMegaMenuItem',
      value: function _firstActiveMegaMenuItem(event, target) {
        if (event.type === 'mouseleave' && target !== event.target) {
          return;
        }

        var menuItemData = $(target).find('.megamenu-item.active').data('link-id');
        $('.SubMegaMenu[data-submenu-id="' + menuItemData + '"]').addClass('active');
      }
    }, {
      ...
      key: '_smallStickyHeaderScroll',
      value: function _smallStickyHeaderScroll() {
        var $headerTop = $('.Header__FlexItem-top');
        var $logo = $('.Header__LogoImage');

        $(window).on('scroll', function () {
          if ($(window).scrollTop() >= 100 && $headerTop.not('.down')) {
            $headerTop.addClass('down');
            $logo.addClass('small');
          } else if ($(window).scrollTop() < 30) {
            $headerTop.removeClass('down');
  ...
  var SlideshowSection = function () {
    ...
    _createClass(SlideshowSection, [{
      ...
      value: function _slideLeave(slide) {
        var content = slide.querySelectorAll('.SectionHeader'),
            buttonWrapper = slide.querySelectorAll('.SectionHeader__ButtonWrapper');
      ...
      value: function _slideEnter(slide) {
        var images = slide.querySelectorAll('.Slideshow__Image'),
            content = slide.querySelectorAll('.SectionHeader'),
            buttonWrapper = slide.querySelectorAll('.SectionHeader__ButtonWrapper');
